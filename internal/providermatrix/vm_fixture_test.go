...
func TestSystemSafetyFixtureRunsCertificateProviderOnPinnedUbuntu(t *testing.T) {
	harness := readRepositoryFile(t, "test", "vagrant", "harness.sh")
	...
	for _, marker := range []string{
		"remotr-vm-certificate-recovery.test",
		"-test.run '^TestCertificateProviderVM$'",
		"REMOTR_CERTIFICATE_VM_PHASE=prepare",
		"REMOTR_CERTIFICATE_VM_PHASE=verify",
		"-test.run '^TestCertificateSecretInterruptedRecoveryVM$'",
	} {
		if !strings.Contains(fixture, marker) {
			t.Errorf("system-safety VM harness is missing %q", marker)
		}
	}
	providerTest := readRepositoryFile(t, "internal", "applicators", "certificates", "vm_recovery_test.go")
	for _, marker := range []string{
		"//go:build vmsafety",
		"func TestCertificateProviderVM",
		"func TestCertificateSecretInterruptedRecoveryVM",
		...
		if !strings.Contains(providerTest, marker) {
			t.Errorf("certificate VM provider test is missing %q", marker)
		}
	}
}
...
func TestSystemSafetyFixtureRunsAppArmorProviderOnPinnedUbuntu(t *testing.T) {
	harness := readRepositoryFile(t, "test", "vagrant", "harness.sh")
	start := strings.Index(harness, "system_safety() {")
	end := strings.Index(harness, "negative_safety() {")
	if start < 0 || end <= start {
	...
	fixture := harness[start:end]
	for _, marker := range []string{
		"remotr-vm-apparmor.test",
		"-test.run '^TestAppArmorProviderVM$'",
	} {
		if !strings.Contains(fixture, marker) {
			t.Errorf("system-safety VM harness is missing %q", marker)
		}
	}
	providerTest := readRepositoryFile(t, "internal", "applicators", "apparmor", "vm_provider_test.go")
	for _, marker := range []string{
		"//go:build vmsafety",
		"func TestAppArmorProviderVM",
		"ResourceKindAppArmorProfile",
		"apparmor_parser",
		"AppArmorEnforce",
		"AppArmorComplain",
		"AppArmorDisabled",
		...
		if !strings.Contains(providerTest, marker) {
			t.Errorf("AppArmor VM provider test is missing %q", marker)
		}
	}
}
...
func TestSystemSafetyFixtureRunsLogrotateProviderOnPinnedUbuntu(t *testing.T) {
	harness := readRepositoryFile(t, "test", "vagrant", "harness.sh")
	start := strings.Index(harness, "system_safety() {")
	end := strings.Index(harness, "negative_safety() {")
	if start < 0 || end <= start {
	...
	fixture := harness[start:end]
	for _, marker := range []string{
		"remotr-vm-logrotate.test",
		"-test.run '^TestLogrotateProviderVM$'",
	} {
		if !strings.Contains(fixture, marker) {
			t.Errorf("system-safety VM harness is missing %q", marker)
		}
	}
	providerTest := readRepositoryFile(t, "internal", "applicators", "logrotate", "vm_provider_test.go")
	for _, marker := range []string{
		"//go:build vmsafety",
		"func TestLogrotateProviderVM",
		"ResourceKindLogrotate",
		"logrotate",
		"--debug",
		"rotate ",
		"compress",
		...
		if !strings.Contains(providerTest, marker) {
			t.Errorf("logrotate VM provider test is missing %q", marker)
		}
	}
}
...
func TestNetworkRecoveryFixtureRunsRouteProviderOnPinnedUbuntu(t *testing.T) {
	harness := readRepositoryFile(t, "test", "vagrant", "harness.sh")
	start := strings.Index(harness, "network_recovery() {")
	end := strings.Index(harness, "system_safety_cleanup() {")
	if start < 0 || end <= start {
	...
	for _, marker := range []string{
		"remotr-vm-network-resources.test",
		"-test.run '^TestRouteProviderVM$'",
	} {
		if !strings.Contains(fixture, marker) {
			t.Errorf("network-recovery VM harness is missing %q", marker)
		}
	}
	providerTest := readRepositoryFile(t, "internal", "applicators", "networkresources", "vm_route_provider_test.go")
	for _, marker := range []string{
		"//go:build vmsafety",
		"func TestRouteProviderVM",
		"ResourceKindRoute",
		"CheckpointRollback",
		"CheckpointDestroy",
		"RollbackTransactional",
		...
		if !strings.Contains(providerTest, marker) {
			t.Errorf("route VM provider test is missing %q", marker)
		}
	}
}
