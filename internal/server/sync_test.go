...
func TestSyncDerivesExecutionLeaseFromAuthenticatedCurrentStateReport(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "unacknowledged current artifact", wantLease: true},
		{name: "acknowledged current artifact", lastReleaseRef: "release-1", lastDigest: true, wantLease: true},
		{name: "stale acknowledged release", lastReleaseRef: "release-0", lastDigest: true},
		{name: "stale reported digest", staleDigest: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.staleDigest {
				reportedDigest = "sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
			}
			bodyFields := map[string]any{
				// The endpoint may have checked this offered artifact but be unable
				// to acknowledge it until the lease permits high-risk Apply.
				"drift": map[string]any{"digest": reportedDigest, "report": json.RawMessage(reportJSON)},
			}
			if tt.lastReleaseRef != "" {
				bodyFields["lastReleaseRef"] = tt.lastReleaseRef
			}
			if tt.lastDigest {
				bodyFields["lastDigest"] = digest
			}
			...
			} else if len(response.ExecutionLeases) != 0 {
				t.Fatalf("stale artifact identity received leases: %+v", response.ExecutionLeases)
			}
		})
	}
}
