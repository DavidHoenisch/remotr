...
func (c *Client) UploadSecretVersion(name, fleet, endpointID string, material []byte) (SecretVersionMetadata, error) {
	...
	request, err := http.NewRequest(http.MethodPost, c.BaseURL+"/v1/admin/secrets/versions?"+query.Encode(), bytes.NewReader(material))
	if err != nil {
...
func (c *Client) ListSecretVersions(name string) ([]SecretVersionMetadata, error) {
	request, err := http.NewRequest(http.MethodGet, c.BaseURL+"/v1/admin/secrets?name="+url.QueryEscape(name), nil)
	if err != nil {
		return nil, err
	}
	...
	if response.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("list secret versions status %d: %s", response.StatusCode, raw)
	}
...
func (c *Client) ActivateSecretVersion(name, version string) (SecretVersionMetadata, error) {
	return c.secretVersionLifecycle("activate", name, version)
}

func (c *Client) RevokeSecretVersion(name, version string) (SecretVersionMetadata, error) {
	return c.secretVersionLifecycle("revoke", name, version)
}

func (c *Client) secretVersionLifecycle(action, name, version string) (SecretVersionMetadata, error) {
	body, err := json.Marshal(map[string]string{"name": name, "version": version})
	if err != nil {
		return SecretVersionMetadata{}, err
	}
	request, err := http.NewRequest(http.MethodPost, c.BaseURL+"/v1/admin/secrets/"+action, bytes.NewReader(body))
	if err != nil {
		return SecretVersionMetadata{}, err
	}
...
func (c *Client) doSecretMetadata(request *http.Request, expectedStatus int, operation string) (SecretVersionMetadata, error) {
	...
	if response.StatusCode != expectedStatus {
		return SecretVersionMetadata{}, fmt.Errorf("%s status %d: %s", operation, response.StatusCode, raw)
	}
	var metadata SecretVersionMetadata
	if err := json.Unmarshal(raw, &metadata); err != nil {
