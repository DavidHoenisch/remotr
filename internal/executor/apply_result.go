...
func (r ApplyResult) Validate() error {
	...
	for _, signal := range r.Activation {
		...
		if (signal.Kind == ActivationReload || signal.Kind == ActivationRestart) && signal.Target == "" {
...
func validActivationKind(kind ActivationKind) bool {
	switch kind {
	case ActivationDaemonReload, ActivationReload, ActivationRestart, ActivationLogoutRequired, ActivationNextBoot, ActivationRebootRequired:
		return true
	default:
		return false
	}
