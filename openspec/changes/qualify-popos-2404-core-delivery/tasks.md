...
- [x] 1.3 Add the missing exact Google Chrome PWA row so an observed `google-chrome` backend publishes the same qualified PopOS PWA capability as Chromium.
...
- [x] 2.3 Exercise both Chromium and Google Chrome through the PopOS VM PWA contract, including drift, Apply, second Check, exact launcher argv, removal, and cleanup.
