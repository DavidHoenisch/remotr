...
All Change-control state is authoritative server-registry data in Postgres rather than process-local cache state. This includes Change requests, approvals, Rollout and Fleet baseline authorizations, Approval policy and warnings, target outcomes and audit history, automatic-promotion policy, Execution leases and attempt accounting, progress, and Break-glass authorizations. A mutating operation commits its complete state transition atomically before the Admin API or authenticated Sync reports success; a failed persistence operation leaves the prior observable state unchanged and issues no lease. Server startup restores this state before serving Admin API or Sync traffic and fails closed instead of silently starting with an empty registry when stored state cannot be loaded or validated. Unexpired leases continue to occupy their approved concurrency slots after restart until completion or expiry, while expired leases are handled by the ordinary lease rules. The memory registry remains a unit-test implementation only.
