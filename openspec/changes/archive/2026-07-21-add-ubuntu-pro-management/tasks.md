...
- [x] 13.3 Update every retained OS-UPM and OS-LPC traceability record with its actual public test, environment selector, and passing disposition; run traceability and provider-matrix audits and keep unproven IDs/rows planned or unadvertised.
...
- [x] 13.5 Run the narrow focused suites, configuration acceptance, authenticated mock integration/e2e checks, every selected credential-free Ubuntu Pro VM selector, `make gosec`, and finally `make test`; record exact commands and results before handoff.
