...
## 7.3 — capability-compatible delivery mutation campaign
...
## 7.5 — traceability and acceptance closeout

- Red: lifecycle audit found the planned `make:mutation-capability-selection` had no runnable target, the mixed load used an obsolete selector name, and OS-AEC-092 named a nonexistent unit selector. Those placeholders prevented an evidence-backed promotion even though the underlying campaign and load had passed.
- Green: the checked-in mutation selector verifies Mewt 3.0.1, regenerates the six-target scope, derives and catches the exact 143 reviewed high-severity mutants, and fails on scope drift or source residue. Traceability now points to that passing selector, `make:load-capability-mixed-400`, and `TestCapabilityMixedPopulationIsBalancedAt400Endpoints`. Every focused selector, benchmark, mutation, and authenticated load requirement passed before OS-AEC-018–026 and OS-AEC-088–092 moved to `verified`; OS-AEC-016–017 were already verified.
- Acceptance: foundation task 10.5 and umbrella task 3.11 are complete, this change has no evidence exception, and structured selector/commit provenance is recorded in `evidence/7.5-traceability-closeout.json`.
