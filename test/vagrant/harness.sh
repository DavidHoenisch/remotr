...
user_safety() {
  ...
  (
    ...
    vagrant ssh -c 'sudo rm -f /tmp/remotr-vm-user-safety.test /tmp/remotr-vm-group-safety.test /tmp/remotr-vm-authorized-key-safety.test'
    vagrant ssh -c '. /etc/os-release; test "$ID" = ubuntu; test "$VERSION_ID" = 24.04'
    ...
    vagrant ssh -c 'sudo rm -f /usr/local/lib/remotr-vm-user-safety.test /usr/local/lib/remotr-vm-group-safety.test /usr/local/lib/remotr-vm-authorized-key-safety.test'
  )
  echo "group, user, and authorized-key safety fixture verified"
}
