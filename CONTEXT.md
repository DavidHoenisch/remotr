...
The Git repository where all desired state lives. Admins merge, branch, and review changes here (GitOps). Modular repos author **modules** and **manifests** under Git; `remotr config compose` generates flat `desired.yaml` and `crons.yaml` artifacts before merge. The server serves those generated bytes at the **Release ref** — it does not merge layers at runtime.
...
**Manifest**:
A YAML source file (`manifest.yaml` or `crons.manifest.yaml`) that lists reusable modules, optional `extends` from another fleet, and per-slice overrides. Composition happens in Git or CI via `remotr config compose`, not on the server.
_Avoid_: Server-side template engine, Ansible role (informal analogy only)

**Deployable artifact**:
A single flat file at a **Fleet path** or **Endpoint override path** (for example `fleets/<fleet>/desired.yaml` or `fleets/<fleet>/crons.yaml`) that the server serves to agents. In modular repos this file is **generated** from manifests and modules; in small repos it may be authored directly. One artifact file per fleet or endpoint override; the server does not merge multiple Git paths at sync time.
...
Optional Git path `endpoints/<endpoint-id>/desired.yaml` for a single machine that must diverge from its fleet. Usually generated from `endpoints/<endpoint-id>/manifest.yaml` (often extending the fleet manifest). When present, the composed artifact replaces (not merges with) the fleet file for that endpoint only.
...
**Expert:** The engineering fleet authors `modules/` slices and lists them in `fleets/engineering-laptops/manifest.yaml`. `remotr config compose` writes `fleets/engineering-laptops/desired.yaml` — that's the **Deployable artifact** at the **Fleet path** the server serves. Arch and Debian laptops share that file; each agent builds **Resolved desired state** locally—pacman stanzas never run on Debian. One laptop needs a unique package? Add `endpoints/<id>/manifest.yaml` (often `extends` the fleet manifest plus extra modules) and compose a full `endpoints/<id>/desired.yaml` — that **Endpoint override path** replaces the fleet artifact for that machine only. PR merges to `main`; a **Git sync** webhook advances the global **Release ref**; agents pull over mTLS and skip work if the artifact digest is unchanged. Each sync runs a **Check** first: if someone installed `nmap` by hand but desired says absent, that's **Drift** reported to the server—separate from an **Apply failure** when a bad sshd regex breaks **Apply**. Production fleets use default **Remediation policy** `auto` to self-heal; the lab fleet uses `report` so drift is visible without ripping out someone's manual install. **Apply** follows **Apply order**; a bad sshd stanza fails **Pre-apply validation** and only that **Resource** is reverted—not nine successful package installs from the same sync.
