...
func TestChangeOutputSupportsHumanAndJSONContracts(t *testing.T) {
	request := admin.ChangeRequest{
		...
		Resources:          []changecontrol.ResourcePlan{{Address: "base/firewall", DesiredHash: "hash-1", Risk: models.RiskConnectivity}},
	...
	for _, want := range []string{"change: change-1", "group: network-transition", "base/firewall  hash-1", "endpoint-a  compatible=true"} {
