...
func configCommand() *cli.Command {
	return &cli.Command{
		...
		Commands: []*cli.Command{
			...
			{
				Name:      "render",
				Usage:     "preview composed deployable artifacts (does not write files)",
				ArgsUsage: "[directory]",
				Description: withExamples("",
					"remotr config render ./remotr-config",
					"remotr config render --fleet engineering",
					"remotr config render --endpoint workstation-42",
					"remotr config render --fleet engineering --output /tmp/desired.yaml"),
				Action: actionConfigRender,
				Flags: append(outputFlags(),
					&cli.StringFlag{Name: "fleet", Usage: "render a specific fleet"},
					&cli.StringFlag{Name: "endpoint", Usage: "render a specific endpoint override"},
					&cli.StringFlag{Name: "output", Usage: "write rendered artifact to file instead of stdout"},
				),
			},
			{
				Name:      "discover",
				Usage:     "list discovered config files for a fleet",
				ArgsUsage: "[directory]",
				Description: withExamples("",
					"remotr config discover --fleet marketing"),
				Action: actionConfigDiscover,
				Flags: append(outputFlags(),
					&cli.StringFlag{Name: "fleet", Usage: "fleet name to discover", Required: true},
				),
			},
			{
				Name:      "validate",
				Usage:     "validate configuration repository sources",
				ArgsUsage: "[directory]",
				Description: withExamples("",
			...
			"remotr config validate --json"),
				...
				Flags: append(outputFlags(),
					&cli.BoolFlag{Name: "skip-render-check", Usage: "do not run composition validation"},
				),
			},
		},
	}
