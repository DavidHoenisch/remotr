...
(
  ...
  CGO_ENABLED=0 go test -mod=vendor -tags=providercontainer -c \
    -o "$work/pacman-provider.test" ./internal/applicators/packages/pacman
)
...
docker run --rm --platform linux/amd64 \
  ...
  --volume "$work/pacman-provider.test:/provider-test:ro" \
  --volume "$root/test/provider-matrix/fixtures/core-packages:/fixtures:ro" \
  "$image" \
  /provider-test -test.run '^TestProviderContainerPacmanContract$' -test.v
