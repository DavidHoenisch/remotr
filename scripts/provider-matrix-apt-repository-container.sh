...
(
  ...
  CGO_ENABLED=0 go test -mod=vendor -tags=providercontainer -c \
    -o "$work/apt-repository-provider.test" ./internal/applicators/aptrepositories
)
...
docker run --rm --platform linux/amd64 \
  ...
  --volume "$work/apt-repository-provider.test:/provider-test:ro" \
  --volume "$root/test/provider-matrix/fixtures/core-packages:/fixtures:ro" \
  "$image" \
  /provider-test -test.run '^TestProviderContainerAPTRepositoryAndKeyContract$' -test.v
